...
/**
 * The destination panel on the 2026 map (#223): a highlighted destination,
 * with the select as the keyboard and large-text equivalent of tapping the
 * plan. The venue is small, so the map never asks where you are or how long
 * the walk is.
 */
...
const to = (page: Page) => page.getByLabel('Go to', { exact: true });

test('an attendee without a plan picks a destination directly, by keyboard', async ({ page }) => {
...
test('the destination follows the next planned talk, then a map link, and the sheet keeps room and devroom together', async ({
  ...
  await page.goto(appUrl(`/map?${WITH_PLAN}`));
