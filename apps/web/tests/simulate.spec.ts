...
test('the simulator fires every reminder tier and logs the banner', async ({ page }) => {
  ...
  // 30-min heads-up at 09:45, leave-now and starting-soon around 10:00, start at 10:15.
  expect(fired).toContain("09:45 Don't miss this");
  expect(fired.some((f) => f.endsWith('Leave now'))).toBe(true);
  expect(fired.some((f) => f.endsWith('Starting soon'))).toBe(true);
  expect(fired.some((f) => f.startsWith('10:15 Starting now'))).toBe(true);
  ...
  await expect(page.getByTestId('sim-latest')).toContainText(
    /STARTS IN|STARTING NOW|Starting now|Leave now|Starting soon/,
  );
