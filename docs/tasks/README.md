...
| ID                                                 | Task                                              | Status                                                                                                                                                                                                                                                                                                                                                                       | Tracks                                                                                                                             |
| -------------------------------------------------- | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| [C-06](C-06-publish-conference-directory.md)       | Publish and validate the canonical room directory | Mechanism implemented, PR #461: `event-sync` derives the directory from `messaging.json` and the bundle, validates it, ships it as the `directory` manifest asset; the PWA reads it on a last-good rule and Settings lists it by alias. The real 2026 file waits on the server, room-policy and route decisions on #166; no 2026 `messaging.json` is invented                | [#166](https://github.com/hanthor/indiafoss-companion/issues/166)                                                                  |
| [C-07](C-07-event-sync-uses-contract.md)           | `event-sync` uses the owned `EventManifest` type  | Implemented, PR #461: every manifest read is validated by the contract, the write names `EVENT_MANIFEST_SCHEMA_VERSION` and asserts `supersedes()`. The spec's 2025 no-op check no longer holds on `main` for an unrelated reason (normalizer 0.2.0 postdates that bundle)                                                                                                   | ADR 0009                                                                                                                           |
| [C-08](C-08-kotlin-fixture-conformance.md)         | Kotlin validates the same golden fixtures         | Implemented, PR #461: `core/Contracts.kt` ports EventManifest and ContactCard validation plus `supersedes`; `ContractConformanceTest` runs one test per fixture case from the shared index and names the five contracts not ported. Not wired into `EventRepository` (C-03 territory)                                                                                        | ADR 0009                                                                                                                           |
| [C-09](C-09-handoff-parse-consolidation.md)        | One handoff parser, both encodings                | Implemented, PR #461: `parseScannedPayload` routes both `indiafoss://` grammars and allow-listed https links through one `handoffToScanned`; `view-session` is a new `session` kind, `import-contact` is named unsupported; a link carrying a credential parameter is refused, not cleaned. Emitted encoding untouched (ADR 0009 q2)                                         | [#31](https://github.com/hanthor/indiafoss-companion/issues/31)                                                                    |
...
| [C-11](C-11-capability-record-release-evidence.md) | Releases record what was actually tested          | Implemented, PR #461: capability namespace in `packages/model/src/capabilities.ts`, `just release-record` emits pins and honest-default claims and refuses an invalid record, first record `docs/evidence/records/release-2026-09-17.json` (nothing at device level), release checklist carries the twelve scenarios, Settings renders readiness from `supportsCapability()` | [#34](https://github.com/hanthor/indiafoss-companion/issues/34)                                                                    |
...
| [X-02](X-02-chat-durable-outbox.md)      | A durable logical outbox with honest delivery states     | Implemented, Chat #64 (`docs/indiafoss/durable-outbox-2026-09-10.md` in that repository); no two-phone evidence                                                                                                                                                       | Chat #48                                                                        |
